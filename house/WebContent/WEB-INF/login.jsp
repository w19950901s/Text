...
$(function(){
	...
	$("#qxid").change(function(){
		$.post(
		 //	就查询街道信息，同时传递过去一个区县的id，同时反馈过来一个街道的集合json数据
		 "jdAction!jdList",
		 {qxid:$(this).val()},
		 //回掉函数
		     function(data){
			     var s = "";
			     for(var i=0;i<data.jdlist.length;i++){//循环取街道同时创建option
...
<tr>
				<td>区县：</td>
				<td><select id="qxid" name="qxid" onChange="selectjd()" style="width:80">
					<option value='0'>不限--</option>
<c:forEach items="${qxlist }" var="qxlix">
<option value="${qxlix.qxid }">${qxlix.qx}</option>
</c:forEach>
...
<table width="88%" cellspacing="0">
				...
				<tr>
					...
					<a href="userAction!fenye?currentPage=${i }">第${i }页</a>&nbsp;
				    </c:forEach>
				    <a href="userAction!fenye?currentPage=1">首页</a>&nbsp;
					<a href="userAction!fenye?currentPage=${p.currentPage-1<=0?1:p.currentPage-1 }">上一页 </a>
					<a href="userAction!fenye?currentPage=${p.currentPage+1>p.allPages?p.allPages:p.currentPage+1 }">下一页</a>&nbsp;
					<a href="userAction!fenye?currentPage=${p.allPages }">尾页</a>
					</td>
				</tr>
			</table>
	</form>
